...
- Featured in: use `references/featured-in.css` — "Featured in" label + auto-scrolling logo marquee (14s, pause-on-hover), matching the live reference exactly. **Duplicate the logo set inside `.logos`** (logos…logos…) so the `-50%` loop is seamless. Logos already in the brand-kit library.
